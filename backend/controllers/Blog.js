...
try {
    ...
    if(FindPost.image)
    {
        ...
        fs.promises.unlink(profilepath).then(()=>console.log("Post Image Deleted")).catch((error)=> console.log("ERROR",error))
    }
...
export const UpdatePost=async(req,res)=>{
    try {
        ...
        await findpost.save()
        return res.status(201).json({success:true,message:"Post Updated Successfully",post:findpost})
    } catch (error) {
        return res.status(500).json({success:false,message:"Internal Server Error"})
}
    }
